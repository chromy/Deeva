...
class DeevaEventDispatcher(object):
    ...
    def stderr(self, stderr):
        self.deeva_main.send('deeva_java', event_obj=DeevaOutputEvent("stderr", stderr))
...
class DeevaOutputEvent(DeevaEvent):
    def __init__(self, output_name, output):
        super(DeevaOutputEvent, self).__init__(output_name)
        self.output = output
    ...
    def format_event(self):
        ...
        lines_json = json.dumps(lines, indent=4*' ')
        array_json_lines = lines_json.split("\n")

        messages.append("event: %s\n" % self.name)
        for array_json_line in array_json_lines:
            messages.append("data: %s\n" % array_json_line)
        messages.append("\n")
