...
Like traditional IDE debugger, user is able to going to next line of code, going into a sub function and returning from a sub function by clicking on the ``Next", ``Into" and ``Out" buttons on the page respectively (see figure~\ref{fig:buttons}).
...
\end{figure}
Deeva can be in different states when debugging a program. The current state of Deeva is shown in the header of the front-end next to Deeva header. Each states is described in more detail below.
\subsubsection{Program not running}
This state indicates that Deeva is just executed from the command line and program is not running yet. In this state, user can inspect various files and set breakpoints accordingly.
\subsubsection{Program is running}
When user runs the program, Deeva is in the running state until it reaches a breakpoint, waiting for Input, or the program is terminated.
Sometimes, it is hardly seen that Deeva is in running state as program executed and reach paused state very quickly.
\subsubsection{Program paused}
Once Deeva reached a breakpoint, it is in paused state. During this state, user may choose various actions e.g. run program, execute next line, or even inspect the current state of the stack and heap.
\subsubsection{Awaiting I/O}
Since Deeva also supports stdIn/stdOut, Deeva is in this state when the program is awaiting for input from the console.
...
\end{figure}
Deeva supports multiple Java files as well as multiple packages. From figure \ref{fig:multipleFile}, the panel that allows different packages or files to be shown is just above the code panel. User can choose to inspect or set breakpoint(s) in any files. However, notice that only those breakpoints in the files that is called by the main class will be stopped at.
...
\end{figure}
Whenever Deeva server is terminated, the front-end will be able to notice it and the modal just like in figure \ref{fig:quitFeature} will be shown. This feature allows user to be noticed that the back-end is terminated and hence, he or she should not continue to debug or expect the correct behaviour.
...
In order to deploy Deeva, one can run {\tt git clone https://github.com/chromy/Deeva.git}, {\tt make deploy}, and {\tt ln -s start\_deeva <somewhere on your path>/deeva} from the command line.
Similarly, to build Deeva, one can run {\tt pip install -r requirements.txt} and {\tt make build} from the command line.
=======
>>>>>>> edca676732dbd0af1e78186f88a196691ec72b03
...
He pointed out that for experienced debuggers our second design would be confusing as it is an unseen choice for debugger bottons.
...
\item Users must be able to see the source code - This was an esential feature that we tried to implement in the early stages of the program. The source code class is loaded in the the code pane, which is displayed using CodeMirror.
\item Users must be able to inspect the current state of the program somehow - This is an important feature for our debugger as we wanted that students understand the workings of Java by visualising how each line influences the program state. We defenetly achieved this by implementing a pleasent and interactive graphical interface that show the links between program stack and heap.
\item Run/Pause/Step into/Step Over - These are essential actions that had to be implemented in order to have a feasible debugger. So, this requirement was quicly added to the tool and is part of the final product.
...
\textbf{Extensions}
Out of the proposed extentions we had time to implement the first two on our extention list:
\begin{enumerate}
...
\item Display of state is graphical - Our tool displays the state of the stack and the heap using a graphical visualisation built in D3 that allows for easy naigation through the stack and the heap objects.
\end{enumerate}
...
During these time we would disscuss the goal for the new cycle, the results of the previous cycle, the highlights of the meeting we had with Tristan and we would allocate the tasks within the next cycle.
...
If we were to do this again we would probably inforce this idea more strongly, luckly, this time we had no issues emerging from it.
...
Also, our most experienced team mambers gave advice and help with other sections of the project.

Finally, through this project we all learned the importance of thourough planning, th importance of keeping every group member updated with the current state of the project and lastly, we understood how software development practices can save lifes (and projects).

\section{Extensions}
After our initial meetings with Tristan we had a plan\ref{sec:initialplan} that included the list of basic features we wanted our debugger to have, followed by a list of extended features ranked in the order we would implement them once the core functionalities are implemented.
...
The idea of using Deeva in two different modes (lecturer mode and user mode) came to us late, but quickle became one of our top-priority extension along with displaying the program state graphically.

These two represented the features that in our opinion, would make Deeva a useful debugger for first year students while the rest of the extensions would not be neccessary in order to achieve that.
...
To sum up, the current level of feature implementation (both core and extenstions) meet the project constrains and represent a functional debugger suitable for teaching introductory Java.
