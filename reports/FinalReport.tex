...
We choose to use JSON and REST partly because it is what we are most used each of us having encountered it at some point in the past but also because it is simpler SOAP/XML and now the defacto standard for new web applications.
...
\emph{Thread Safety}
As will be described in further detail in the \emph{Middleware} section, \texttt{py4j} and \texttt{Flask} are thread-safe, however access to objects in Java are not i.e. the Debugger class that the class will interact with needs to have synchronisation primitives.
