...
This is probably due to the fact that they were not properly introduced to debugging tools or they found them too difficult to understand. So, with Deeva we are hoping to `inspire' new programmers to embrace debugging tools and to use them not only  for fixing bugs but also for understanding the workings of different programming languages.   
...
\subsection{Start/Stop/Step/Step Into/Step Return}
\subsection{Arguments}
\subsection{Assertion}
\subsection{Stdin/Stdout/Stderr}
\subsection{Not Compiled}
...
To meet the requirements of the core functionality, we had designed the user interface with respect to each of the features.
The interface is essentially divided into two parts.
The left half contains mainly the source code and the right half contains the stack and heap graph.
In the left part, a tab-like bar allows users to input and to delete arguments easily. 
Additionally, a series of buttons enables users to compile the function, start debugging, go to next line, go into a sub-function, return from a sub-function.
A drop-down list displays all available classes from the package so that user could select to go to a specific class.
Lastly, a code pane displays the source code.
In the right part, the user interface contains a console output area which allows users to see any Stdout or Stderr, and also takes Stdin.
A detailed diagram is also included to show the current state of the program in a form of stack and heap.
...
\todo{This became important when mid way through the project we realised that the resolution of the projectors in the lecture halls was too low to see both the code and the state on the same screen.}
...
For the code pane we used CodeMirror\todo[inline]{reference} and we built the state pane using D3 we chose these libraries over alternatives because they are both active projects with good documentation. 
...
We used a number of techniques to evaluate as we built it Deeva.
...
We also used a few other metrics, some custom and some off-the-shelf which could be automatically run over the repository to measure specific aspects of Deeva’s performance one of these was Time to First Request.
